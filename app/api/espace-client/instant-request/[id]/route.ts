...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      clientHasReviewed,
      plombierHasReviewed,
      etaMinutes: data.etaMinutes ?? undefined,
      etaAt: data.etaAt?.toDate?.()?.toISOString?.() ?? null,
      arrivedAt: data.arrivedAt?.toDate?.()?.toISOString?.() ?? null,
      clientReadyAt: data.clientReadyAt?.toDate?.()?.toISOString?.() ?? null,
