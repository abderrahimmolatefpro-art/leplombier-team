...
**Note pour Gmail :** Vous devez utiliser un "Mot de passe d'application" et non votre mot de passe Gmail normal. Pour créer un mot de passe d'application :
...
L'URL de l'endpoint API sera :
- **En développement :** `http://localhost:3000/api/webhook/client`
- **En production :** `https://votre-domaine.vercel.app/api/webhook/client`

## Code du formulaire pour WordPress/Elementor

### Option 1 : Code HTML/JS à intégrer dans Elementor (HTML Widget)
...
<body>
  ...
  <script>
    (function() {
      // Configuration - REMPLACEZ par votre URL d'API
      const API_URL = 'https://votre-domaine.vercel.app/api/webhook/client';
      // Optionnel : ajoutez votre clé API si vous l'avez configurée
      const API_KEY = ''; // Laissez vide si vous n'utilisez pas de clé API
...
### Option 2 : Code simplifié pour Elementor (sans HTML wrapper)

Si vous utilisez Elementor, vous pouvez copier seulement le contenu entre `<body>` et `</body>` dans un widget HTML personnalisé.

## Instructions d'installation

1. **Dans WordPress/Elementor :**
   - Ajoutez un widget "HTML" ou "Code personnalisé"
   - Collez le code HTML/JS ci-dessus
   - **IMPORTANT :** Remplacez `API_URL` dans le JavaScript par votre URL d'API réelle

2. **Configuration de l'email :**
   - Ajoutez les variables SMTP dans `.env.local` (voir section Configuration requise)
   - Les emails seront envoyés à `ogincema@gmail.com` automatiquement

3. **Sécurité (optionnel) :**
   - Générez une clé API secrète (ex: `openssl rand -hex 32`)
   - Ajoutez-la dans `.env.local` comme `WEBHOOK_API_KEY`
   - Ajoutez-la dans le JavaScript du formulaire comme `API_KEY`

## Test du formulaire

1. Remplissez le formulaire sur votre site WordPress
2. Vérifiez que le client apparaît dans le CRM
3. Vérifiez que l'email de notification est reçu à `ogincema@gmail.com`

## Personnalisation

Vous pouvez personnaliser les couleurs dans le CSS pour correspondre exactement à votre charte graphique :
